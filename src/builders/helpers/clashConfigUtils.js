export function emitClashRules(rules = [], translator, resolveTargetName) {
    if (!translator) {
        throw new Error('emitClashRules requires a translator function');
    }
    const getTarget = typeof resolveTargetName === 'function'
        ? resolveTargetName
        : (outbound) => translator('outboundNames.' + outbound);
    const results = [];
    ...
    rules
        ...
        .forEach(rule => {
            rule.src_ip_cidr.forEach(cidr => {
                if (!cidr) return;
                results.push(`SRC-IP-CIDR,${cidr},${getTarget(rule.outbound)}`);
            });
        });
    rules
        ...
        .forEach(rule => {
            rule.domain_suffix.forEach(suffix => {
                results.push(`DOMAIN-SUFFIX,${suffix},${getTarget(rule.outbound)}`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.domain_keyword.forEach(keyword => {
                results.push(`DOMAIN-KEYWORD,${keyword},${getTarget(rule.outbound)}`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.site_rules.forEach(site => {
                results.push(`RULE-SET,${site},${getTarget(rule.outbound)}`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.ip_rules.forEach(ip => {
                results.push(`RULE-SET,${ip}-ip,${getTarget(rule.outbound)},no-resolve`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.ip_cidr.forEach(cidr => {
                results.push(`IP-CIDR,${cidr},${getTarget(rule.outbound)},no-resolve`);
            });
        });
